	...
	public String withdrawMoney(String custId, Double amount) throws ValidationException {
		...
		try{
			if(null == custId){
				...
				accountDetails.setAccountNumber(customerDetails.getAccountNumber());
				accountDetails.setBalance(customerDetails.getBalance());
				accountDetails.setTransactionMode("D");
			}

			if(null!=amount && amount>0){
				if(null!=customerDetails.getBalance() && customerDetails.getBalance() >0){
					Double currentBalance=customerDetails.getBalance()-amount;
					rowUpdated=customerServiceDao.UpdateCustomerBalance(String.valueOf(custId),currentBalance);
					customerServiceDao.insertTransaction(accountDetails);	
				}
				...
				throw new ValidationException ("Amount can not be null or zero");
			}
		}catch(ValidationException ex){

			throw new ValidationException("Exception occured while withdrawing money ::"+ex.getMessage());
		}
		if(rowUpdated>0){
			return "Withdraw money transaction successfully updated";
		}else{
			return "Withdraw money transaction Failed";
		}
	}
